...
with tab3:
    ...
    if factor_prices.empty or factor_prices.isna().all().all():
        ...
        for col, d in [("1D",1),("1W",5),("1M",21),("3M",63),("6M",126),("1A",252),("3A",756),("5A",1260)]:
            f[col] = factor_prices.apply(lambda x: ret(x, d))
        f["YTD"] = factor_prices.apply(ret_ytd)

        def style_row(row):
            if row.name in FACTOR_COMPARISON: return ["background-color:#1e1e1e;color:#ccc"] * len(row)
            return ["background-color:#000;color:white"] * len(row)

        def highlight_extremes(s):
            valid = s.dropna()
            if valid.empty: return [""] * len(s)
            max_val, min_val = valid.max(), valid.min()
            return ["background-color:#003300;color:#00FF00;font-weight:bold" if v == max_val
                    else "background-color:#330000;color:#ff4422;font-weight:bold" if v == min_val
                    else "" for v in s]

        st.dataframe(
            f.round(2).style.apply(style_row, axis=1)
...
with tab6:
    st.markdown(
        ...
        '<p style="color:#666;font-size:0.85em;margin-top:0;">P/E live da worldperatio.com · Medie storiche da worldperatio.com</p>'
        '<p style="color:#444;font-size:0.78em;margin-top:4px;">📅 Dati storici aggiornati a: <b style="color:#ff9900">Febbraio 2026</b></p>',
        unsafe_allow_html=True)
    period_sel = st.radio("Confronta con media storica",
                           options=["Last 1Y","Last 3Y","Last 5Y","Last 10Y","Last 20Y"], index=2, horizontal=True)
    ...
    if pe_hist is None:
        ...
        if _pe_fallback:
            st.markdown(
                ...
                f'⚠️ worldperatio non raggiungibile per: <b>{", ".join(_pe_fallback)}</b> — fallback yfinance</div>',
                unsafe_allow_html=True)
        etf_sectors = {
        ...
        for etf, sector_name in etf_sectors.items():
            ...
            dev_live= round((pe_now - avg_pe) / std_dev, 2) if pe_now and avg_pe and std_dev and std_dev > 0 else None
            ...
            rows.append({"ETF": etf, "Settore": sector_name, "P/E Live": pe_now,
                          f"Avg P/E ({period_sel})": avg_pe, "Std Dev": std_dev,
                          "Dev σ (live)": dev_live, "Valutazione": label, "_color": color})
        ...
        fig_pe = go.Figure()
        fig_pe.add_trace(go.Bar(x=comp_df["ETF"], y=comp_df["Dev σ (live)"], marker_color=bar_colors_pe,
        ...
        for y, c, txt in [(0,"#444444",None),(1.5,"#ffaa00","Overvalued"),(2.5,"#ff4422","Expensive"),(-0.5,"#00ff55","Cheap")]:
            kwargs = dict(y=y, line_color=c, line_width=1)
            if txt: kwargs.update(line_dash="dot", annotation_text=txt, annotation_font_color=c, annotation_position="right")
            fig_pe.add_hline(**kwargs)
        fig_pe.update_layout(height=320, paper_bgcolor="#000000", plot_bgcolor="#000000",
            font=dict(color="white", size=11),
        ...
        def style_valuation(val):
            return {"Cheap":"color:#00ff55;font-weight:bold","Fair":"color:#aaaaaa",
                    "Moderato":"color:#ffff44;font-weight:bold","Overvalued":"color:#ffaa00;font-weight:bold",
                    "Expensive":"color:#ff4422;font-weight:bold","N/D":"color:#555555"}.get(str(val), "")
        def style_dev(val):
            try:
                v = float(val)
        ...
        st.dataframe(
            display_df.style.map(style_valuation, subset=["Valutazione"]).map(style_dev, subset=["Dev σ (live)"])
            .format({"P/E Live": lambda x: f"{x:.2f}" if x else "N/D",
        ...
        st.markdown("""
        <div style="background:#0d0d0d;border:1px solid #222;border-radius:8px;
                    padding:10px 20px;margin-top:8px;font-size:0.82em;color:#888;display:flex;gap:20px;flex-wrap:wrap;">
            <span><b style="color:#00ff55">Cheap</b> — sotto la media storica</span>
            <span><b style="color:#aaaaaa">Fair</b> — entro 0.5σ</span>
            <span><b style="color:#ffff44">Moderato</b> — tra 0.5σ e 1.5σ</span>
            <span><b style="color:#ffaa00">Overvalued</b> — tra 1.5σ e 2.5σ</span>
            <span><b style="color:#ff4422">Expensive</b> — oltre 2.5σ</span>
        </div>""", unsafe_allow_html=True)
...
with tab7:
    ...
    if th_prices.empty:
        st.error("Impossibile scaricare i dati.")
        st.stop()

    c1, c2, c3 = st.columns([2, 2, 1])
    with c1:
        tf_label = st.selectbox("Timeframe analisi", ["1D","1W","1M","3M","6M","YTD","1A","2A"], index=4, key="tem_tf_main")
    with c2:
    ...
    for sector, tickers, bm_ticker in TEMATICI_STRUCT:
        bm_ret = safe_ret(th_prices[bm_ticker], tf_days_main) if bm_ticker in th_prices.columns else None
        for ticker in tickers:
            if ticker not in th_prices.columns: continue
            ...
            rows_tem.append({
                "Gruppo": sector, "Ticker": ticker, "Tema": TEMATICI_DESCRIPTIONS.get(ticker, ticker),
                "BM": bm_ticker, "BM ret": round(bm_ret, 2) if bm_ret and not np.isnan(bm_ret) else np.nan,
                "1D": round(safe_ret(s, 1), 2), "1W": round(safe_ret(s, 5), 2),
                "1M": round(safe_ret(s, 21), 2), "3M": round(safe_ret(s, 63), 2),
                "6M": round(safe_ret(s, 126), 2), "YTD": round(safe_ret(s, None), 2),
                "1A": round(safe_ret(s, 252), 2), "2A": round(safe_ret(s, 504), 2),
    ...
    total_etf, total_all = len(valid_ret), len(tem_df)
    ...
    kpi_data = [
        ...
        (k2, f"Positivi {tf_label}", f"{pos_count}/{total_etf} ({pos_pct}%)", "#00ff55" if pos_pct >= 50 else "#ff4422"),
        (k3, f"Top {tf_label}", f"{top_row['Ticker']} ({top_row[tf_label]:+.1f}%)" if top_row is not None else "—", "#00ff55"),
        (k4, f"Worst {tf_label}", f"{bot_row['Ticker']} ({bot_row[tf_label]:+.1f}%)" if bot_row is not None else "—", "#ff4422"),
    ]
    for col, label, val, color in kpi_data:
        col.markdown(f'<div style="background:#0d0d0d;border:1px solid #222;border-radius:8px;padding:10px 14px;margin-bottom:6px;">'
    ...
    if missing_n > 0:
        st.markdown(f'<div style="background:#1a1000;border:1px solid #332200;border-radius:6px;'
                    f'padding:6px 14px;margin-bottom:8px;font-size:0.78em;color:#aa7700;">'
                    f'⚠️ <b>{missing_n} ETF esclusi</b> su <b>{tf_label}</b>: {", ".join(missing_tickers)}</div>',
                    unsafe_allow_html=True)
    ...
    for sector, tickers, bm_ticker in TEMATICI_STRUCT:
        ...
        coerenza_data.append({"Gruppo": sector, "Pct_pos": round((grp > 0).sum() / len(grp) * 100, 1),
                               "n_pos": int((grp > 0).sum()), "n_tot": int(len(grp)), "BM_ret": bm_val})
    cdf_plot = pd.DataFrame(coerenza_data)

    col_chart, col_panel = st.columns([3, 2])
    with col_chart:
        bar_bm_colors = ["#00cc44" if (not np.isnan(r["BM_ret"]) and r["BM_ret"] >= 0) else "#cc2200"
        ...
        fig_coh  = go.Figure()
        for _, r in cdf_plot.iterrows():
            if np.isnan(r["BM_ret"]): continue
            fig_coh.add_shape(type="line", x0=r["Gruppo"], x1=r["Gruppo"], y0=0, y1=r["BM_ret"],
                line=dict(color="#00cc44" if r["BM_ret"] >= 0 else "#cc2200", width=8, dash="solid"), opacity=0.45)
        fig_coh.add_trace(go.Bar(x=cdf_plot["Gruppo"], y=cdf_plot["BM_ret"].fillna(0),
            marker=dict(color=bar_bm_colors, opacity=0.70, line=dict(color="#333", width=1)), width=0.40,
            text=[f"{v:+.1f}%" if not np.isnan(v) else "n/d" for v in cdf_plot["BM_ret"]],
            textposition="outside", textfont=dict(color="#888888", size=8), name=f"BM {tf_label}",
            hovertemplate=f"<b>%{{x}}</b><br>Benchmark {tf_label}: %{{y:+.1f}}%<extra></extra>"))
        if show_bm:
            breadth_col   = ["#00ff55" if r["Pct_pos"] >= 60 else "#ffff44" if r["Pct_pos"] >= 40 else "#ff4422"
                             for _, r in cdf_plot.iterrows()]
            hover_diamond = [f"<b>{r['Gruppo']}</b><br>BM {tf_label}: {r['BM_ret']:+.1f}%<br>"
                             f"Breadth: {r['Pct_pos']:.0f}% ({r['n_pos']}/{r['n_tot']})"
                             for _, r in cdf_plot.iterrows()]
            fig_coh.add_trace(go.Scatter(x=cdf_plot["Gruppo"], y=cdf_plot["BM_ret"].fillna(0),
                mode="markers", marker=dict(symbol="diamond", size=8, color=breadth_col,
                line=dict(color="#ffffff", width=1.2)), name="Breadth ◆",
                hovertemplate="%{customdata}<extra></extra>", customdata=hover_diamond))
        fig_coh.add_hline(y=0, line_color="#555", line_width=1.5)
        fig_coh.update_layout(height=500, paper_bgcolor="#000", plot_bgcolor="#000",
            font=dict(color="white", size=9),
            title=dict(text=f"Ritorno benchmark {tf_label}  |  ◆ = breadth tematici" if show_bm else f"Ritorno benchmark {tf_label}",
                        font=dict(size=9, color="#555"), x=0, xanchor="left"),
            xaxis=dict(tickangle=-25, gridcolor="#0a0a0a", tickfont=dict(size=8)),
            yaxis=dict(range=[y_bot, y_top], gridcolor="#111", ticksuffix="%", zeroline=False,
                       tickfont=dict(size=8), title=None),
            legend=dict(font=dict(size=8), bgcolor="rgba(0,0,0,0)", orientation="h", y=1.02, x=1, xanchor="right"),
            margin=dict(l=45, r=40, t=30, b=60), barmode="overlay")
        st.plotly_chart(fig_coh, use_container_width=True)

    with col_panel:
        n_bm_pos       = (cdf_plot["BM_ret"] > 0).sum()
        n_bm_tot       = len(cdf_plot["BM_ret"].dropna())
        global_breadth = round(pos_pct, 1)
        avg_bm_ret     = round(cdf_plot["BM_ret"].mean(), 1)
        if n_bm_pos >= 9:   regime_txt, regime_color, regime_icon = "RISK ON DIFFUSO",  "#00ff55", "🟢"
        elif n_bm_pos >= 6: regime_txt, regime_color, regime_icon = "MOMENTUM MISTO",   "#ffff44", "🟡"
        elif n_bm_pos >= 3: regime_txt, regime_color, regime_icon = "RISK OFF PARZIALE","#ffaa00", "🟠"
        else:               regime_txt, regime_color, regime_icon = "RISK OFF DIFFUSO", "#ff4422", "🔴"
        ...
        all_positive, all_negative = len(bm_negativi) == 0, len(bm_positivi) == 0
        if not all_negative:
            leader_df, leader_title, leader_note = bm_positivi.head(3), "▲ Leader benchmark", ""
        else:
            leader_df = sorted_cdf.head(3)
            leader_title = "▲ Meno peggio"
            leader_note  = f'<div style="color:#555;font-style:italic;font-size:0.75em;margin-top:2px;">tutti negativi su {tf_label}</div>'
        if not all_positive:
            corr_df, corr_title, corr_note = bm_negativi.head(3), "▼ Benchmark in correzione", ""
        else:
            corr_df = sorted_cdf.tail(3).sort_values("BM_ret")
            corr_title = "▼ Gruppi meno forti"
            corr_note  = f'<div style="color:#555;font-style:italic;font-size:0.75em;margin-top:2px;">tutti positivi su {tf_label}</div>'
        divergenze = []
        for _, r in cdf_plot.iterrows():
            if np.isnan(r["BM_ret"]): continue
            if r["BM_ret"] > 0 and r["Pct_pos"] < 40:
                divergenze.append(f'<span style="color:#ffaa00">⚡ <b>{r["Gruppo"]}</b></span>: '
                                  f'BM {r["BM_ret"]:+.1f}% ma breadth {r["Pct_pos"]:.0f}% → rally non confermato')
            elif r["BM_ret"] <= 0 and r["Pct_pos"] >= 60:
                divergenze.append(f'<span style="color:#44aaff">⚡ <b>{r["Gruppo"]}</b></span>: '
                                  f'BM {r["BM_ret"]:+.1f}% ma breadth {r["Pct_pos"]:.0f}% → tematici reggono')
        leader_html = "".join([f'<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #1a1a1a;">'
                                f'<span style="color:#aaa;font-size:0.82em">{r["Gruppo"]}</span>'
                                f'<span style="color:#00ff55;font-weight:bold;font-size:0.82em">{r["BM_ret"]:+.1f}%</span></div>'
                                for _, r in leader_df.iterrows()]) or \
                      '<span style="color:#444;font-style:italic;font-size:0.80em">—</span>'
        corr_color = "#ff4422" if not all_positive else "#ffaa00"
        corr_html  = "".join([f'<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #1a1a1a;">'
                               f'<span style="color:#aaa;font-size:0.82em">{r["Gruppo"]}</span>'
                               f'<span style="color:{corr_color};font-weight:bold;font-size:0.82em">{r["BM_ret"]:+.1f}%</span></div>'
                               for _, r in corr_df.iterrows()]) or \
                     '<span style="color:#444;font-style:italic;font-size:0.80em">—</span>'
        div_html = "<br>".join(divergenze) if divergenze else \
                   '<span style="color:#444;font-style:italic;font-size:0.80em">Nessuna divergenza significativa</span>'
        st.markdown(
            f'<div style="background:#080808;border:1px solid #222;border-radius:10px;padding:16px 18px;height:100%;font-family:monospace;">'
            f'<div style="border-bottom:1px solid #222;padding-bottom:10px;margin-bottom:12px;">'
            f'<div style="color:#555;font-size:0.70em;letter-spacing:0.1em;text-transform:uppercase;">Regime tematico — {tf_label}</div>'
            f'<div style="color:{regime_color};font-size:1.25em;font-weight:bold;margin-top:4px;">{regime_icon} {regime_txt}</div>'
            f'<div style="color:#444;font-size:0.75em;margin-top:2px;">BM positivi: {n_bm_pos}/{n_bm_tot} &nbsp;·&nbsp; '
            f'Breadth: {global_breadth}% &nbsp;·&nbsp; BM medio: {avg_bm_ret:+.1f}%</div></div>'
            f'<div style="margin-bottom:12px;"><div style="color:#555;font-size:0.68em;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:4px;">{leader_title}</div>'
            f'{leader_note}{leader_html}</div>'
            f'<div style="margin-bottom:12px;"><div style="color:#555;font-size:0.68em;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:4px;">{corr_title}</div>'
            f'{corr_note}{corr_html}</div>'
            f'<div><div style="color:#555;font-size:0.68em;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:6px;">⚡ Divergenze rilevate</div>'
            f'<div style="font-size:0.78em;line-height:1.7;">{div_html}</div></div></div>',
            unsafe_allow_html=True)
    ...
    def highlight_best(s):
        styles = [""] * len(s)
        valid  = s.dropna()
        if valid.empty: return styles
        max_v, min_v = valid.max(), valid.min()
        for i, v in enumerate(s):
            if pd.isna(v):      styles[i] = "color:#333"
            elif v == max_v:    styles[i] = "background-color:#003300;color:#00FF00;font-weight:bold"
            elif v == min_v:    styles[i] = "background-color:#330000;color:#ff4422;font-weight:bold"
            elif v > 0:         styles[i] = "color:#88cc88"
            else:               styles[i] = "color:#cc6644"
        return styles
    ...
    def color_vs_bm(val):
        ...
        except Exception:    return "color:#333"

    def fmt_pct(x):
        try:
            f_val = float(x)
            return "—" if np.isnan(f_val) else f"{f_val:+.1f}%"
        except Exception: return "—"

    styled_tem = (view_df.style.apply(highlight_best, subset=tf_cols)
    ...
    st.dataframe(styled_tem, use_container_width=True, hide_index=True,
                 column_config={"Gruppo": st.column_config.TextColumn("Gruppo", width="medium"),
                                "Tema":   st.column_config.TextColumn("Tema",   width="medium"),
                                "vs BM":  st.column_config.TextColumn(f"vs BM ({tf_label})", width="small")})
    st.markdown(f'<div style="background:#0d0d0d;border:1px solid #222;border-radius:6px;padding:8px 16px;margin-top:4px;font-size:0.78em;color:#666;">'
                f'<b style="color:#00ff55">Verde</b> = top per colonna · <b style="color:#ff4422">Rosso</b> = worst per colonna · '
                f'<b style="color:#ff9900">vs BM</b> = delta tematico − benchmark su {tf_label}</div>', unsafe_allow_html=True)

    st.markdown("---")
    st.markdown(f"### 3 · Top & Bottom 5 — {tf_label}")
    ranked = tem_df.dropna(subset=[tf_label]).sort_values(tf_label, ascending=False)
    top5, bot5 = ranked.head(5), ranked.tail(5).sort_values(tf_label)
    c_top, c_bot = st.columns(2)

    def mini_bar(df_slice, ascending=False, title=""):
        colors_mb = ["#00ff55" if v >= 0 else "#ff4422" for v in df_slice[tf_label]]
        fig_mb = go.Figure(go.Bar(x=df_slice[tf_label], y=df_slice["Ticker"] + " · " + df_slice["Tema"],
            orientation="h", marker=dict(color=colors_mb, line=dict(color="#333", width=1)),
            text=[f"{v:+.1f}%" for v in df_slice[tf_label]], textposition="outside",
            textfont=dict(color="white", size=10), hovertemplate="<b>%{y}</b><br>%{x:.1f}%<extra></extra>"))
        fig_mb.add_vline(x=0, line_color="#444", line_width=1)
        fig_mb.update_layout(height=200, paper_bgcolor="#000", plot_bgcolor="#000", font=dict(color="white", size=9),
            ...
            margin=dict(l=10, r=60, t=35, b=20), showlegend=False)
        return fig_mb

    with c_top: st.plotly_chart(mini_bar(top5, title="🏆 Top 5"), use_container_width=True)
    with c_bot: st.plotly_chart(mini_bar(bot5, ascending=True, title="💀 Bottom 5"), use_container_width=True)

    st.markdown("---")
    st.markdown("### 4 · Scatter quadranti")
    sc_tf_opts = ["1D","1W","1M","3M","6M","YTD","1A","2A"]
    sc_c1, sc_c2, sc_c3 = st.columns([2, 1, 1])
    with sc_c1: sc_tf = st.selectbox("Timeframe scatter", sc_tf_opts, index=sc_tf_opts.index(tf_label), key="tem_scatter_tf")
    with sc_c2: label_mode = st.radio("Etichette", ["Ticker","Tema"], horizontal=True, key="tem_sc_lbl")
    with sc_c3: filter_scatter = st.checkbox("Solo gruppo selezionato", value=False, key="tem_sc_filter")

    rows_sc = []
    for sector, tickers, bm_ticker in TEMATICI_STRUCT:
        bm_r = safe_ret(th_prices[bm_ticker], TF_DAYS[sc_tf]) if bm_ticker in th_prices.columns else np.nan
        for ticker in tickers:
            if ticker not in th_prices.columns: continue
            s     = th_prices[ticker].dropna()
            abs_r = safe_ret(s, TF_DAYS[sc_tf])
            if np.isnan(abs_r): continue
            delta = (abs_r - bm_r) if not np.isnan(bm_r) else np.nan
            rows_sc.append({"Gruppo": sector, "Ticker": ticker, "Tema": TEMATICI_DESCRIPTIONS.get(ticker, ticker),
                             "BM": bm_ticker, "BM_ret": bm_r, "abs_ret": abs_r, "delta_bm": delta})
    ...
    if sc_df.empty:
        ...
        group_colors = {s: c for s, c in zip([g for g, _, _ in TEMATICI_STRUCT],
                        ["#ff9900","#00ff55","#44aaff","#ff4422","#ffff44","#bb44ff",
                         "#00ffcc","#ff66cc","#88cc88","#cc8844","#4488ff","#ff8844"])}
        fig_sc = go.Figure()
        for sector in sc_df["Gruppo"].unique():
            ...
            for _, r in sub.iterrows():
                ...
                hover_text.append(f"<b>{lbl}</b><br>Gruppo: {sector}<br>Ret {sc_tf}: {r['abs_ret']:+.1f}%<br>BM ({r['BM']}): {bm_str}<br>vs BM: {r['delta_bm']:+.1f}%")
            fig_sc.add_trace(go.Scatter(x=sub["abs_ret"], y=sub["delta_bm"], mode="markers+text", name=sector,
                marker=dict(size=10, color=color, opacity=0.85, line=dict(color="#111", width=1)),
                text=labels, textposition="top center", textfont=dict(size=8, color="#cccccc"),
        ...
        for qx, qy, qlabel, qcolor, qanchor in [
            (sc_df["abs_ret"].max()-pad_x,  sc_df["delta_bm"].max()-pad_y, "LEADER",     "#00ff55", "top right"),
            (sc_df["abs_ret"].min()+pad_x,  sc_df["delta_bm"].max()-pad_y, "ALPHA PURO", "#44aaff", "top left"),
            (sc_df["abs_ret"].max()-pad_x,  sc_df["delta_bm"].min()+pad_y, "BETA PURO",  "#ffaa00", "bottom right"),
            (sc_df["abs_ret"].min()+pad_x,  sc_df["delta_bm"].min()+pad_y, "EVITARE",    "#ff4422", "bottom left"),
        ]:
            fig_sc.add_annotation(x=qx, y=qy, text=qlabel, showarrow=False,
                font=dict(color=qcolor, size=10, family="Courier New"),
                opacity=0.45, xanchor=qanchor.split()[1], yanchor=qanchor.split()[0])
        fig_sc.update_layout(height=500, paper_bgcolor="#000000", plot_bgcolor="#000000",
            font=dict(color="white", size=10),
            title=dict(text=f"Scatter {sc_tf} — X: ritorno assoluto  |  Y: delta vs benchmark",
                        font=dict(size=10, color="#666")),
            xaxis=dict(title=f"Ritorno assoluto {sc_tf} (%)", gridcolor="#1a1a1a", ticksuffix="%", zeroline=False),
            yaxis=dict(title=f"Delta vs benchmark ({sc_tf}) %", gridcolor="#1a1a1a", ticksuffix="%", zeroline=False),
            legend=dict(font=dict(size=8), bgcolor="rgba(0,0,0,0.5)", bordercolor="#333", borderwidth=1,
                        orientation="v", x=1.01, y=1),
            margin=dict(l=60, r=160, t=50, b=60), hoverlabel=dict(bgcolor="#111", font_size=11))
        st.plotly_chart(fig_sc, use_container_width=True)
        st.markdown("""
        <div style="background:#0d0d0d;border:1px solid #222;border-radius:8px;
                    padding:10px 20px;margin-top:4px;font-size:0.80em;color:#888;display:flex;gap:24px;flex-wrap:wrap;">
            <span><b style="color:#00ff55">LEADER</b> — positivo E sovraperforma il benchmark</span>
            <span><b style="color:#44aaff">ALPHA PURO</b> — negativo ma batte il benchmark</span>
            <span><b style="color:#ffaa00">BETA PURO</b> — positivo ma sotto il benchmark</span>
            <span><b style="color:#ff4422">EVITARE</b> — negativo E sottoperforma</span>
        </div>""", unsafe_allow_html=True)
    st.markdown('<div style="background:#0a0a0a;border:1px solid #1a1a1a;border-radius:8px;padding:10px 20px;margin-top:16px;font-size:0.78em;color:#555;">'
                '<b style="color:#ff9900">vs BM</b> = differenza ritorno tematico − ritorno benchmark di gruppo (stesso TF)</div>',
                unsafe_allow_html=True)
