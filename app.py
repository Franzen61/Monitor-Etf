...
def clean_num(val):
    if pd.isna(val) or str(val).strip().upper() in ['NONE', 'NAN', '']: return 0.0
...
def load_live_data():
    ...
    try:
        ...
        # Pulizia nomi colonne (rimuove spazi bianchi extra)
...
if df_set is not None:
    ...
    if menu == "Monitor Settori":
        ...
        # Basandoci sulla diagnostica: Ticker(0), Ra/momentum(7), Coerenza trend(8), Classifica(9), Δ_RS (5d)(10), Situazione(11), Operatività(12)
    ...
    # --- ANALISI FATTORI ---
        ...
        # Evidenzia solo le colonne performance (escludendo Ticker e Prezzo)
    ...
    # --- SERIE STORICHE (MOTORE) ---
        ...
        # Rinomina Close in SPY come richiesto
        df_mot = df_mot.rename(columns={'Close': 'SPY'})
        df_mot['Date'] = pd.to_datetime(df_mot['Date'], dayfirst=True, errors='coerce')
        # Filtro 2025
